...
/// at compile time (from factory methods or card code).
///
...
mod tests {
    ...
    fn filter_basic_types() {
        ...
        assert!(Filter::any_permanent().matches_permanent_ignore_controller(&bear));
    }
    ...
    #[test]
    fn filter_or_combinator_with_filter_new() {
        let bear = make_creature("Bear", 2, 2, KeywordAbilities::empty());
        let land = make_land("Forest", bear.controller);

        let creature_or_land = Filter::new("creature or land", Predicate::creature().or(Predicate::land()));
        assert!(creature_or_land.matches_permanent_ignore_controller(&bear));
        assert!(creature_or_land.matches_permanent_ignore_controller(&land));
    ...
    #[test]
    fn filter_basic_land_with_subtypes() {
        let owner = PlayerId::new();
        let mut card = CardData::new(ObjectId::new(), owner, "Forest");
        card.card_types = vec![CardType::Land];
        card.supertypes = vec![SuperType::Basic];
        ...
        let basic_forest = Filter::new("basic Forest card",
            Predicate::HasSuperType(SuperType::Basic)
                .and(Predicate::land())
                .and(Predicate::HasSubType(SubType::Forest)));
        assert!(basic_forest.matches_card_ignore_controller(&card));
        let basic_plains = Filter::new("basic Plains card",
            Predicate::HasSuperType(SuperType::Basic)
                .and(Predicate::land())
                .and(Predicate::HasSubType(SubType::Plains)));
        assert!(!basic_plains.matches_card_ignore_controller(&card));
    }
    ...
    fn filter_subtype() {
        ...
        let elf_filter = Filter::new("Elf", Predicate::HasSubType(SubType::Elf));
        let warrior_filter = Filter::new("Warrior", Predicate::HasSubType(SubType::Warrior));
        let goblin_filter = Filter::new("Goblin", Predicate::HasSubType(SubType::Goblin));
        assert!(elf_filter.matches_permanent_ignore_controller(&perm));
        assert!(warrior_filter.matches_permanent_ignore_controller(&perm));
        assert!(!goblin_filter.matches_permanent_ignore_controller(&perm));
    }
